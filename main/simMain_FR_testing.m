...
%% initialization
clearvars -except xx countX; close all;
% add all the files in the repo to running path, assuming you are in the gridIntegration repo
...
% 				[c0, fdOpt] = feederSetup(fName, fdSetup,overwrite);
				...
				if isfield(c0,'weights')
					...
					SystemSize=real(SystemSize(1,:));
					SystemSize(:,find(SystemSize==0))=[];
					for ii=1:length(c0.pvsystem)
						c0.pvsystem(ii).kVA=SystemSize(ii);
						c0.pvsystem(ii).pmpp=SystemSize(ii);
				...
				[loadProf, rawProf] = getLoadProfile(fName,conf.loadProfileId);
				...
				for penId = 1:length(conf.PVLevel)
					...
					if isfield(c0,'weights')
% 					LOAD=[]; 	ind=[];
% 					%Add in solar and load profiles
% 					LoadVect=cell2mat([c_pen.load(:).Kvar]);
% 					LoadShape=[c_pen.loadshape(end).Mult];
% 					LoadMat=LoadShape*LoadVect';
% 					%add load to bus
% 					ln=[c_pen.load(:).bus1];
% 					for idx=1:length(ln)
% 					ind(idx)=find(strcmpi(res.nodeName,ln(idx)));
% 					end
% 					for idx=1:length(res.nodeName)
% 						if ~isempty(find(ind==idx))
% 						LOAD(idx,:)=LoadMat(:,find(ind==idx));
% 						else
% 						LOAD(idx,:)=zeros(length(LoadMat),1);
% 						end
% 					end
% 					res.LoadMat=LOAD;
% 					
...
% 					ind=[];
% 					%Add in solar
% 					SolarVect=cell2mat([c_pen.pvsystem(:).KVA]);
% 					SolarShape=[c_pen.loadshape(1:end-1).Mult];
% 					SolarMat=[SolarShape{:}].*repmat(SolarVect',2880,1);
% 					%add load to bus
% 					sn=[c_pen.pvsystem(:).bus1];
% 					for idx=1:length(sn)
% 					ind(idx)=find(strcmpi(res.nodeName,sn(idx)));
% 					end
% 					for idx=1:length(res.nodeName)
% 						if ~isempty(find(ind==idx))
% 						Solar(idx,:)=SolarMat(:,find(ind==idx));
% 						else
% 						Solar(idx,:)=zeros(length(SolarMat),1);
% 						end
					end
% 					res.SolarMat=Solar;
					res.RedTime=c_pen.SimTime;
% 					res.PhaseReduced=c_pen.PhaseReduction;
					res.buslist=c_pen.buslist.id;
					end
					...
					if ExportToNAS==1
						mkdir(['C:\Users\Zactus\gridIntegration\TimeTest\'])
						movefile(fn,['C:\Users\Zactus\gridIntegration\TimeTest\sim_' num2str(xx) '_' kk '.mat']);
					end
				end
			end
		end
