...
[topo,generation]=topology_detect_large_orig(topo,generation,Ybus,parent,Node_number);
topo_view=topo;
...
trfBus(ii,jj)=strtok(Buses(jj),'\.');
trf_kV(ii,jj)=circuit.transformer(ii).kv(jj);
end
end
...
trf_kV=trf_kV(:,2:end); trf_kV=trf_kV(:); trf_kV=trf_kV(find(~cellfun(@isempty,trf_kV))); 
...
% 		innerCB_parents=cell2mat(generation(criticalNumbers(j),4));
% 		VectorOfCommonParents=(CB_parents(find(ismemberi(CB_parents,innerCB_parents))));
% 		[~,NewCriticalNumber]=max(cell2mat(generation(VectorOfCommonParents,5)));
...
% 		New_criticalNumbers1=[New_criticalNumbers1, VectorOfCommonParents(NewCriticalNumber)];
% % 		else
% % 			break
% % 		end
% 	end
% end
% criticalNumbers=vertcat(criticalNumbers, New_criticalNumbers1');
% toc
tic
for k=1:nn
CB_parents=cell2mat(generation(criticalNumbers(k),4));
...
%%save for testing
% save('IEEE8500_saved_workspace.mat')
...
parent=1;
topo(parent,1)=parent;
[topo,generation]=topology_detect_large_orig(topo,generation,Ybus_reduced,parent,Node_number);
topo_view=topo;
topo_view(find(topo_view(:,1)==0)',:)=[];
c_new=0;
