function [circuit, circuit_orig, times] = reducingFeeders_ckt5(pathToFile,criticalBuses,savePath)
...
for ii=1:length(circuit.transformer)
	if ii>1
		circuit.transformer(ii).kVS=[7.1996 7.1996];
	end
	circuit.transformer(ii).imag=0;
	circuit.transformer(ii).noloadloss=0;
	circuit.transformer(ii).loadloss=0;
end
	circuit=rmfield(circuit,'capacitor');
...
% 	XfrmrToRmv(1:3)=[]; XfrmrToRmv(end-8:end)=[];
% 	XfrmrToRmv=[];
