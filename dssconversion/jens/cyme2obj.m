function [ ob ] = cyme2obj( o,flgTest,NetworkID)
...
%         o.Loads.LocationToModelSpotLoads{i,1}='F'; % 'F' = load is modeled at the 'from' node, otherwise load is modeled at the 'to' node
        ...
        if ~isempty(index2)
            ...
            if ~isempty(Value2)
                Phase1Value2=sum(str2double(o.LoadsAll.Value2(index2)));
                n1Value2=length(o.LoadsAll.Value2(index2));
                pf1=Phase1Value2/n1Value2/100; % pf=P/S; S^2=P^2+Q^2; Q=sqrt(P^2/pf^2-P^2)
                o.Loads.Phase1Kvar{i,1}=num2str(sqrt(Phase1Kw^2/pf1^2-Phase1Kw^2));
            else
        ...
        if ~isempty(index2)
            ...
            Value2=Value2(Value2~=0);
            if ~isempty(Value2)
                Phase2Value2=sum(str2double(o.LoadsAll.Value2(index2)));
                n2Value2=length(o.LoadsAll.Value2(index2));
                pf2=Phase2Value2/n2Value2/100; % pf=P/S; S^2=P^2+Q^2; Q=sqrt(P^2/pf^2-P^2)
                o.Loads.Phase2Kvar{i,1}=num2str(sqrt(Phase2Kw^2/pf2^2-Phase2Kw^2));
            else
        ...
        if ~isempty(index2)
            ...
            Value2=Value2(Value2~=0);
            if ~isempty(Value2)
                Phase3Value2=sum(str2double(o.LoadsAll.Value2(index2)));
                n3Value2=length(o.LoadsAll.Value2(index2));
                pf3=Phase3Value2/n3Value2/100; % pf=P/S; S^2=P^2+Q^2; Q=sqrt(P^2/pf^2-P^2)
                o.Loads.Phase3Kvar{i,1}=num2str(sqrt(Phase3Kw^2/pf3^2-Phase3Kw^2));
            else
