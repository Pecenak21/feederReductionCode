...
if ~isempty(strfind(tline,'[LOAD EQUIVALENT]'))
   ...
   if ~isempty(strfind(tline,'FORMAT_LOADEQUIVALENT=NodeID,Format,Value1A,Value1B,Value1C,Value2A,Value2B,Value2C'))
       tline = fgetl(fid);
       while isempty(strfind(tline,'[OVERHEADLINE SETTING]')) 
...
% FUSE SETTING data
fid2 = fopen([FileName '_FUSESETTING.txt'], 'w');
if ~isempty(strfind(tline,'[FUSE SETTING]'))
   tline = fgetl(fid);
   if ~isempty(strfind(tline,'FORMAT_FUSESETTING=SectionID,Location,EqID,DeviceNumber,CoordX,CoordY,ClosedPhase,Locked,RC,NStatus,DemandType,Value1A,Value2A,Value1B,Value2B,Value1C,Value2C,Value1ABC,Value2ABC,TCCID,PhPickup,GrdPickup,Alternate,PhAltPickup,GrdAltPickup,FromNodeID,FaultIndicator,Strategic,RestorationMode,ConnectionStatus,ByPassOnRestoration,Reversible'))
       tline = fgetl(fid);
       while isempty(strfind(tline,'[RECLOSER SETTING]')) 
             yFUSESETTING=tline;
             fprintf(fid2,'%s\r\n',yFUSESETTING);
             tline = fgetl(fid);
       end
