...
flgTest=1; % set to '1' to select some reduced versions of the files that describe the system


%% Get data 
cd('D:\Sync Software\Other Code\SystemConversion\Convert2dss\jens');
curdir = pwd;
fns = {[curdir '\Input_CYME\CYMload.txt'];...
        [curdir '\Input_CYME\CYMnetwork.txt'];...
        [curdir '\Input_CYME\CYMequipment.txt']};
flgChopFile=0;
% fns={}; % activate if larger files are already chopped

% load excel data
d1 = cyme2obj(fns,flgChopFile,flgTest);
% d2 = excel2obj(fns{2});
% d3 = excel2obj(fns{3});
% d4 = excel2obj(fns{4});
% d5 = excel2obj(fns{5});
