function[ExecutionFlag] = GetCYMData_equip(FilePath)
...
if ~isempty(strfind(tline,'[SPACING TABLE FOR LINE]'))
   tline = fgetl(fid);
   if ~isempty(strfind(tline,'FORMAT_SPACINGTABLEFORLINE=ID,GMDPh-Ph,GMDPh-N,AvgPhCondHeight,AvgNeutralHeight,PosOfCond1_X,PosOfCond1_Y,PosOfCond2_X,PosOfCond2_Y,PosOfCond3_X,PosOfCond3_Y,PosOfNeutralCond_X,PosOfNeutralCond_Y,PosOfCond1_C2_X,PosOfCond1_C2_Y,PosOfCond2_C2_X,PosOfCond2_C2_Y,PosOfCond3_C2_X,PosOfCond3_C2_Y,PosOfNeutralCond_N2_X,PosOfNeutralCond_N2_Y,BundleDistance,NBPhasesPerCircuit,NBConductorsPerPhase,NBNeutrals,TowerType,DistanceA,DistanceB,DistanceC,DistanceD,DistanceE,ConductorStatusN1,ConductorStatusN2,FootingResistanceN1,FootingResistanceN2,TowerSpanN1,TowerSpanN2,Comments'))
       tline = fgetl(fid);
       while isempty(strfind(tline,'[DOUBLECIRCUITSPACING]')) 
...
if ~isempty(strfind(tline,'[NON IDEAL CONVERTER]'))
   ...
   if ~isempty(strfind(tline,'FORMAT_NONIDEALCONVERTER=ID,PulseNumber,KVA,KV,SymbolOnID,SymbolOffID,Comments'))
       tline = fgetl(fid);
       while isempty(strfind(tline,'[FREQUENCY SOURCE]')) 
...
if ~isempty(strfind(tline,'[WIND MODEL]'))
   ...
   if ~isempty(strfind(tline,'FORMAT_WINDMODEL=ID,FromFile,FileName,Comments'))
       tline = fgetl(fid);
       while isempty(strfind(tline,'[INSOLATION MODEL]')) 
             yWINDMODEL=tline;
             fprintf(fid2,'%s\r\n',yWINDMODEL);
             tline = fgetl(fid);
       end
