...
elseif strcmp(strSystem,'SCE_Centaur')
    strDirectory='C:\Work\Projects\2013\1665-CEC_Forecasting\Simulations\OpenDSS\Centaur-Jens';
    strDirectory_output=strDirectory;
...
elseif strcmp(strSystem,'SCE_Durox')
    strDirectory='C:\Work\Projects\2013\1665-CEC_Forecasting\Simulations\OpenDSS\Durox-Jens';
    strDirectory_output=strDirectory;
...
if strcmp(strSource,'cyme')
    ...
    [d1,node_bk] = cyme2obj(o,flgTest,NetworkID);
    glc=d1.LineCode;
    gwd=d1.WireData;
    glg=d1.LineGeometry;
...
elseif strcmp(strSystem,'SCE_Durox')
    c1 = circuit_changes_Durox(c1, node_bk);
elseif strcmp(strSystem,'SCE_Centaur')
    c1 = circuit_changes_Centaur(c1, node_bk);
end
...
% validatepower('kw',0,'',[1 0 0 0 0],c1,0,p1);
% validatepower('kvar',0,'',[1 0 0 0 0],c1,0,p1);
validateSC(c1,d1,p1);

%% Run Daily simulations for the DistribuTECH BRAZIL presentation
c1 = DistribuTECH(c1, p1, 1);%adjuct the system for daily simulations
p1 = dsswrite(c1,strSystem,1,strSystem);%add made changes to the .dss files
Commands = {};%any additional changes to the circuit on the fly (e.g. setting values, etc.)
SimName = ('Case 1');%simulation title, will appear on the figures and figure names when saved
stepsize = [num2str(c1.loadshape(1).sInterval) 's'];%time will advance based on stepsize (e.g. 1h=3600s, 15m=900s or 30s) for each solve
% stepsize = '30';
SimPer = 24;%simulaiton period in hours (e.g 24)
PVSys = {c1.pvsystem.daily};% names of all PV loadshapes in the circuit
% PVSys = {c1.pvsystem.daily};
NumPts = length(c1.loadshape(1).Mult);%number of 'solves' (e.g. 24 for a daily simulaiton with 1h stepsize or 2880 for 30sec stepsize)
DailySimulations(Commands, SimName, stepsize, SimPer, p1, PVSys, NumPts);

c1 = DistribuTECH(c1, p1, 2);%adjuct the system for daily simulations
p1 = dsswrite(c1,strSystem,1,strSystem);%add made changes to the .dss files
SimName = ('Case 2');%simulation title, will appear on the figures and figure names when saved
DailySimulations(Commands, SimName, stepsize, SimPer, p1, PVSys, NumPts);
